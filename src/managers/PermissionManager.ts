...
export class PermissionManager {
  ...
  async hasPermission(userId: string, voiceChannelId: string, guildId: string): Promise<boolean> {
    ...
    if (channelData.ownerId === userId) return true;

    // Check if user is a co-owner
    const coowners = this.bot.db.getCoOwners(voiceChannelId);
    if (coowners.some(co => co.userId === userId)) return true;
    ...
    try {
      ...
      if (cownerRole && member.roles.cache.has(cownerRole.id)) return true;
  ...
  async canPerformOwnerAction(userId: string, voiceChannelId: string, guildId: string): Promise<boolean> {
    if (this.isOwner(userId, voiceChannelId)) return true;
   ...
   */
  async applyChannelPermissions(voiceChannel: VoiceChannel) {
   ...
   */
  async lockChannel(voiceChannel: VoiceChannel) {
    try {
   ...
   */
  async unlockChannel(voiceChannel: VoiceChannel) {
    try {
      await voiceChannel.permissionOverwrites.edit(voiceChannel.guild.roles.everyone, {
   ...
   */
  async hideChannel(voiceChannel: VoiceChannel) {
    try {
      await voiceChannel.permissionOverwrites.edit(voiceChannel.guild.roles.everyone, {
        ViewChannel: false,
      });
   ...
   */
  async unhideChannel(voiceChannel: VoiceChannel) {
    try {
      await voiceChannel.permissionOverwrites.edit(voiceChannel.guild.roles.everyone, {
   ...
   */
  async resetAllPermissions(voiceChannel: VoiceChannel) {
    try {
