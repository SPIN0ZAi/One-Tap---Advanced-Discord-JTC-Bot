...
class ModalHandler {
        ...
        try {
            ...
            if (!voiceChannel) {
                return await interaction.reply({
                    embeds: [(0, embeds_1.createErrorEmbed)('Voice channel not found.')],
                    ephemeral: true,
                });
            }
            // Check permissions
            ...
            switch (interaction.customId) {
                ...
                default:
                    await interaction.reply({
                        embeds: [(0, embeds_1.createErrorEmbed)('Unknown modal action.')],
                        ephemeral: true,
                    });
            }
        }
    ...
    async handleChangeName(interaction, voiceChannel) {
        ...
        try {
            ...
            await interaction.editReply({
                embeds: [(0, embeds_1.createSuccessEmbed)(`Channel name changed to **${newName}**`)],
            });
            await this.bot.stickyManager.refreshStickyMessage(voiceChannel.id);
        }
        catch (error) {
            await interaction.editReply({
                embeds: [(0, embeds_1.createErrorEmbed)('Failed to change channel name. Please try again.')],
            });
        }
    }
    async handleSetLimit(interaction, voiceChannel) {
        ...
        if (isNaN(limit) || limit < 0 || limit > 99) {
            return await interaction.reply({
                embeds: [(0, embeds_1.createErrorEmbed)('Invalid limit. Please enter a number between 0 and 99.')],
                ephemeral: true,
            });
        }
        await interaction.deferReply({ ephemeral: true });
        try {
            ...
            await interaction.editReply({
                embeds: [(0, embeds_1.createSuccessEmbed)(message)],
            });
            await this.bot.stickyManager.refreshStickyMessage(voiceChannel.id);
        }
        catch (error) {
            await interaction.editReply({
                embeds: [(0, embeds_1.createErrorEmbed)('Failed to set user limit. Please try again.')],
            });
        }
    }
    async handleSetCustomGif(interaction, voiceChannel) {
        ...
        if (!gifUrl.startsWith('http://') && !gifUrl.startsWith('https://')) {
            return await interaction.editReply({
                embeds: [(0, embeds_1.createErrorEmbed)('Invalid URL. Please provide a valid HTTP/HTTPS link.')],
            });
        }
        try {
            ...
            await interaction.editReply({
                embeds: [(0, embeds_1.createSuccessEmbed)(`🎨 Custom GIF saved! It will appear in all your future channels. Leave and rejoin to see it now!`)],
            });
            ...
            await interaction.editReply({
                embeds: [(0, embeds_1.createErrorEmbed)('Failed to set custom GIF. Please try again.')],
            });
        }
    }
    async handlePermitUser(interaction, voiceChannel) {
        ...
        if (!targetId) {
            return await interaction.editReply({
                embeds: [(0, embeds_1.createErrorEmbed)('Invalid user or role. Please use a mention or valid ID.')],
            });
        }
        ...
        if (!targetType) {
            return await interaction.editReply({
                embeds: [(0, embeds_1.createErrorEmbed)('Could not find user or role with that ID.')],
            });
        }
        try {
            // Add to whitelist
            ...
            await interaction.editReply({
                embeds: [(0, embeds_1.createSuccessEmbed)(`${targetMention} has been permitted to join the channel.`)],
            });
            await this.bot.stickyManager.refreshStickyMessage(voiceChannel.id);
        }
        catch (error) {
            await interaction.editReply({
                embeds: [(0, embeds_1.createErrorEmbed)('Failed to permit user/role. Please try again.')],
            });
        }
    }
    async handleBlacklistUser(interaction, voiceChannel) {
        ...
        try {
            ...
            const targetMention = targetType === 'user' ? `<@${targetId}>` : `<@&${targetId}>`;
            await interaction.editReply({
                embeds: [(0, embeds_1.createSuccessEmbed)(`${targetMention} has been rejected from the channel.`)],
            });
            await this.bot.stickyManager.refreshStickyMessage(voiceChannel.id);
        }
        catch (error) {
            await interaction.editReply({
                embeds: [(0, embeds_1.createErrorEmbed)('Failed to reject user/role. Please try again.')],
            });
        }
    }
    async handleTransferOwnership(interaction, voiceChannel) {
        ...
        if (!userId) {
            return await interaction.editReply({
                embeds: [(0, embeds_1.createErrorEmbed)('Invalid user. Please use a mention or valid user ID.')],
            });
        }
        ...
        try {
            ...
            if (!member) {
                return await interaction.editReply({
                    embeds: [(0, embeds_1.createErrorEmbed)('User not found in this server.')],
                });
            }
            // Transfer ownership
            ...
            await interaction.editReply({
                embeds: [(0, embeds_1.createSuccessEmbed)(`Ownership transferred to <@${userId}> successfully!`)],
            });
            await this.bot.stickyManager.refreshStickyMessage(voiceChannel.id);
        }
        catch (error) {
            await interaction.editReply({
                embeds: [(0, embeds_1.createErrorEmbed)('Failed to transfer ownership. Please try again.')],
            });
        }
    }
    async handleSlowmode(interaction, voiceChannel) {
        ...
        if (isNaN(slowmode) || slowmode < 0 || slowmode > 21600) {
            return await interaction.reply({
                embeds: [(0, embeds_1.createErrorEmbed)('Invalid slowmode. Please enter a number between 0 and 21600 seconds.')],
                ephemeral: true,
            });
        }
        await interaction.deferReply({ ephemeral: true });
        ...
        try {
            ...
            if (textChannel && textChannel.type === discord_js_1.ChannelType.GuildText) {
                await textChannel.setRateLimitPerUser(slowmode);
            ...
            await interaction.editReply({
                embeds: [(0, embeds_1.createErrorEmbed)('Failed to set slowmode. Please try again.')],
            });
        }
    }
    async handleBitrate(interaction, voiceChannel) {
        ...
        if (isNaN(bitrate) || bitrate < 8 || bitrate > 384) {
            return await interaction.reply({
                embeds: [(0, embeds_1.createErrorEmbed)('Invalid bitrate. Please enter a number between 8 and 384 kbps.')],
                ephemeral: true,
            });
        }
        await interaction.deferReply({ ephemeral: true });
        ...
        if (bitrate > 64) {
            const warning = (0, embeds_1.createWarningEmbed)(`Setting bitrate above 64kbps requires a boosted server or premium features. ` +
        ...
        try {
            ...
            await interaction.editReply({
                embeds: [(0, embeds_1.createSuccessEmbed)(`Bitrate set to **${bitrate} kbps**`)],
            });
            await this.bot.stickyManager.refreshStickyMessage(voiceChannel.id);
        }
        catch (error) {
            ...
            await interaction.editReply({
                embeds: [(0, embeds_1.createErrorEmbed)(errorMessage)],
            });
        }
    }
    async handleSetStatus(interaction, voiceChannel) {
        ...
        try {
            if (status.trim().length === 0) {
                ...
                await interaction.editReply({
                    embeds: [(0, embeds_1.createSuccessEmbed)('Voice channel status cleared.')],
                });
            }
            else {
                await voiceChannel.setStatus(status);
                await interaction.editReply({
                    embeds: [(0, embeds_1.createSuccessEmbed)(`Voice channel status set to: **${status}**`)],
                });
            }
            await this.bot.stickyManager.refreshStickyMessage(voiceChannel.id);
        }
            ...
            await interaction.editReply({
                embeds: [(0, embeds_1.createErrorEmbed)('Failed to set status. This feature may require a boosted server.')],
            });
        }
    }
    async handleManageCoOwners(interaction, voiceChannel) {
        ...
        if (action === 'list') {
            ...
            if (coowners.length === 0) {
                return await interaction.editReply({
                    embeds: [(0, embeds_1.createSuccessEmbed)('No co-owners set for this channel.')],
                });
            }
            let list = '**Co-Owners:**\n';
            ...
            return await interaction.editReply({
                embeds: [(0, embeds_1.createSuccessEmbed)(list)],
            });
        }
        if (action === 'clear') {
            this.bot.db.clearCoOwners(voiceChannel.id);
            return await interaction.editReply({
                embeds: [(0, embeds_1.createSuccessEmbed)('All co-owners have been removed.')],
            });
        }
        const targetId = this.extractId(targetStr);
        if (!targetId) {
            return await interaction.editReply({
                embeds: [(0, embeds_1.createErrorEmbed)('Please provide a valid user mention or ID.')],
            });
        }
        if (action === 'add') {
            // Verify user exists
            try {
                ...
                await interaction.editReply({
                    embeds: [(0, embeds_1.createSuccessEmbed)(`<@${targetId}> added as co-owner.`)],
                });
                await this.bot.stickyManager.refreshStickyMessage(voiceChannel.id);
            }
            catch (error) {
            ...
            await interaction.editReply({
                embeds: [(0, embeds_1.createSuccessEmbed)(`<@${targetId}> removed as co-owner.`)],
            });
            await this.bot.stickyManager.refreshStickyMessage(voiceChannel.id);
        }
        else {
            await interaction.editReply({
                embeds: [(0, embeds_1.createErrorEmbed)('Invalid action. Use: add, remove, list, or clear')],
            });
        }
    }
    extractId(str) {
    ...
    async determineTargetType(guildId, targetId) {
        try {
